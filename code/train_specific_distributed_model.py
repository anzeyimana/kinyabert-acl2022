# Launch scripts
#
# nohup python3 train_specific_distributed_model.py  -g 1 -p 3 -s 1 --use-pos-aware-rel=1 --use-tupe-rel=0 --batch-size=20 --accumulation-steps=128 --number-of-load-batches=384 --num-iters=200000 --warmup-iter=2000 --peak-lr=0.0004 --wd=0.01  &> log.out & tail -f log.out
#
# nohup python3 train_specific_distributed_model.py  -g 1 -p 3 -s 1 --use-pos-aware-rel=0 --use-tupe-rel=1 --batch-size=6 --accumulation-steps=428 --number-of-load-batches=8560 --num-iters=200000 --warmup-iter=2000 --peak-lr=0.0004 --wd=0.01  &> log.out & tail -f log.out
...
def train_loop(args, rank, scaler, device, data_loder, start_line, tot_num_affixes, model, optimizer, lr_scheduler, save_file_path, accumulation_steps, loop, num_loops, bar, total_steps, total_loss, stem_loss, affix_loss):
    ...
    if (rank == 0):
        ...
        model.train()
    return total_steps, total_loss, stem_loss, affix_loss
...
def train_fn(rank, args):
    ...
    curr_save_file_path = (home_path+"data/morpho_attentive_model_base_2021-04-19.pt")
    prev_save_file_path = (home_path+"data/morpho_attentive_model_base_2021-04-19.pt")

    # Switch to TUPE version file
    if args.use_tupe_rel_pos_bias:
        curr_save_file_path = (home_path+"data/morpho_attentive_model_base_2021-07-12.pt")
        prev_save_file_path = (home_path+"data/morpho_attentive_model_base_2021-07-12.pt")
    ...
    num_train_loops = math.ceil(num_iters * accumulation_steps / number_of_load_batches)
    ...
    if (rank == 0):
        ...
        print('number_of_load_batches: ', number_of_load_batches)
        print('accumulation_steps: ', accumulation_steps)
        print('batch_size: ', batch_size)
        print('effective_batch_size: ', batch_size*accumulation_steps)
    ...
    with progressbar.ProgressBar(initial_value=lr_scheduler.num_iters, max_value=lr_scheduler.end_iter, redirect_stdout=True) as bar:
        ...
        for loop in range(curr_loops, num_train_loops):
            ...
            kb_dataset = KBCorpusDataset(args, kb_vocab, affix_set_vocab, bpe_encoder,
                                         ...
                                         number_of_load_batches * batch_size,
            ...
            kb_data_loader = DataLoader(kb_dataset, batch_size=batch_size, collate_fn=morpho_seq_collate_wrapper, shuffle=True)

            if (rank == 0):
                print(time_now(), 'Memory status: ', psutil.virtual_memory(), flush=True)

            (total_steps,
             total_loss, stem_loss, affix_loss) = train_loop(args, rank,
                                                             scaler,
                                                             device,
                                                             kb_data_loader,
                                                             start_line,
                                                             ...
                                                             bar, total_steps,
                                                             total_loss, stem_loss, affix_loss)
